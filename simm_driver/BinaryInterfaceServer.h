...
public:
     ...
     */
    BinaryInterfaceServer(const uint16_t port) : Port(port)
    {
        bind();
...
private:
    ...
    void bind()
    {
        ...
        if (bindHandle <= 0)
        {
            throw std::runtime_error("BinaryInterfaceServer::bind: Error creating Stream Socket. Errno: " + std::to_string(Error) + ": " + std::string(strerror(Error)));
        }
        ...
        if (rc != 0)
        {
            throw std::runtime_error("BinaryInterfaceServer::bind: Error setting keepalive. Errno: " + std::to_string(Error) + ": " + std::string(strerror(Error)));
        }
        ...
        if (rc != 0)
        {
            throw std::runtime_error("BinaryInterfaceServer::bind: Error setting Timeout. Errno: " + std::to_string(Error) + ": " + std::string(strerror(Error)));
        }
        ...
        if (rc != 0)
        {
            throw std::runtime_error("BinaryInterfaceServer::bind: Error binding port " + std::to_string(Port) + ". Errno: " + std::to_string(Error) + ": " + std::string(strerror(Error)));
        }
        ...
        if (rc != 0)
        {
            throw std::runtime_error("BinaryInterfaceServer::bind: Error listening on port " + std::to_string(Port) + ". Errno: " + std::to_string(Error) + ": " + std::string(strerror(Error)));
        }
    }
};
